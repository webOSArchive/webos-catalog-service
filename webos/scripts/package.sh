...
Description: webOS Account setup for the webOS App Museum. Sign in or create a community account that becomes your device profile.
Source: {"Type": "Application", "Title": "webOS Account", "FullDescription": "Optional community account setup for the webOS App Museum (appcatalog.webosarchive.org). Adds a webOS Account app to the Launcher; signing in makes your App Museum account the device profile. Installs a small patch redirecting the long-dead HP account service to the community backend (original files are preserved and restored on uninstall)."}
